...
if (envConfig.error) {
  ...
  console.log('Successfully loaded variables');
}
...
function checkAuth(req, res, next) {
  ...
  console.log('Auth check failed for path:', req.originalUrl);

  // If the request is from client-side JS (like fetch), send a JSON error
  if (req.xhr || req.headers.accept.includes('json')) {
    return res.status(401).json({ error: 'Unauthorized. Please log in.', redirectTo: '/login.html' });
  } else {
    // Otherwise, it's a direct browser request, so redirect the whole page
